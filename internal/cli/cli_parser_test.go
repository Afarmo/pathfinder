...
func TestCliParser(t *testing.T) {
	randomTest := []struct {
		...
		{"non integer train count", []string{"pwd", "filePath.map", "start", "end", "not int"}, true},
		{"too many arguments", []string{"pwd", "filePath.map", "start", "end", "5", "some thing extra"}, true},
		{"invalid extension", []string{"pwd", "filePath", "start", "end", "5"}, true},
		{"too few arguments", []string{"pwd", "filePath.map", "end", "5"}, true},
